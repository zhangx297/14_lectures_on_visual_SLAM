...
int main(int argc, char **argv){
	...
	//quiryIdx,trainIdx匹配点
        Point2d pt1 = pixel2cam(keypoints_1[m.queryIdx].pt, K); //pixel2cam像素坐标转换为归一化坐标
	Mat y1 = (Mat_<double>(3, 1) << pt1.x, pt1.y, 1);
	Point2d pt2 = pixel2cam(keypoints_2[m.trainIdx].pt, K);
	Mat y2 = (Mat_<double>(3, 1) << pt2.x, pt1.y, 1);
	Mat d = y2.t() * t_x * R * y1; //.t()转置
...
void find_feature_matches(const Mat &img_1, const Mat &img_2, std::vector<KeyPoint> &keypoints_1, std::vector<KeyPoint> &keypoints_2, std::vector<DMatch> &matches){
    ...
    //当描述子之间的距离大于两倍的最小距离时，即认为匹配有误，但有时候最小距离会非常小，设置一个经验值为30作为下限
    for (int i = 0; i < descriptors_1.rows; i++){
