...
void ComputeORB(const cv::Mat &img,vector<cv::KeyPoint> &keypoint, vector<DescType> &descriptors);
...
int main(int argc, char **argv){
    ...
    //detect FAST keypoints1
    chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
    vector<cv::KeyPoint> keypoints1;
    cv::FAST(first_image, keypoints1, 40); //特征提取
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors){
    const int half_patch_size = 8; //检测关键点选取的图像块边长
    const int half_boundary = 16; //描述子所选取图像块区域
    int bad_points = 0;
	...
	kp.pt.x >= img.cols - half_boundary || kp.pt.y >= img.rows - half_boundary){
	    ...
	    for (int dx = -half_patch_size; dx < half_patch_size; ++dx){
		    ...
		    uchar pixel = img.at<uchar>(kp.pt.y + dy, kp.pt.x + dx); //获得像素值
		    m10 += dx * pixel; //x轴质心位置
		    m01 += dy * pixel;
		}
	    }
