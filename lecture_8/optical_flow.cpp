...
class OpticalFlowTracker{
	public:
		OpticalFlowTracker(const Mat &img1_, const Mat &img2_, const vector<KeyPoint> &kp1_,
				vector<KeyPoint> &kp2_, vector<bool> &success_, bool inverse_ = true, bool has_initial_ = false):
...
int main(int argc, char **argv){
    ...
    Mat img2_CV;
    cv::cvtColor(img1, img2_CV, COLOR_GRAY2BGR);
...
void OpticalFlowSingleLevel(const Mat &img1, const Mat &img2, const vector<KeyPoint> &kp1, vector<KeyPoint> &kp2, vector<bool> &success, bool inverse, bool has_initial){
    ...
    //调用parallel_for_并行调用OpticalFlowTracker::calculateOpticalFlow,该函数计算制定范围内特征点的光流
    parallel_for_(Range(0, kp1.size()), std::bind(&OpticalFlowTracker::calculateOpticalFlow, &tracker, placeholders::_1));//range():从指定的第一个值开始，并在到达指定的第二个值后终止
}
...
void OpticalFlowTracker::calculateOpticalFlow(const Range &range){
	...
	if (has_initial){
	    dx = kp2[i].pt.x - kp.pt.x; //第i个点在第二张图中的位置与第一章图中位置的插值
...
void OpticalFlowMultiLevel(const Mat &img1, const Mat &img2, const vector<KeyPoint> &kp1, vector<KeyPoint> &kp2, vector<bool> &success, bool inverse){
    ...
    for (int i = 0; i < pyramids; i++){
	    ...
	    cv::resize(pyr1[i-1],img2_pyr, cv::Size(pyr2[i - 1].cols * pyramid_scale, pyr2[i-1].rows * pyramid_scale));
